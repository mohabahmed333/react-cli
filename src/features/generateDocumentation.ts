...
function getFileType(filePath: string, baseDir: string) {
...
function extractMetadata(content: string, isTypeScript: boolean) {
    const metadata: any = {};
    ...
    if (exportMatch) {
        metadata.exportType = exportMatch[1];
...
function parseInlineTypeProps(typeString: string) {
    const props: any = {};
...
function addInlineProp(line: string, props: any) {
...
function parseFile(filePath: string, config: any) {
...
function generateStorybookDoc(fileInfo: any, docsConfig: any) {
...
function getDefaultValueForProp(prop: string, propData: any): string | undefined {
...
function generateMarkdownDoc(fileInfo: any, docsConfig: any) {
    ...
    if (fileInfo.type === 'component' && fileInfo.metadata.props) {
        ...
        mdContent += `|------|------|-------------|\n`;
        Object.entries(fileInfo.metadata.props).forEach(([name, details]: any) => {
...
export async function generateDocumentation(options: any, config: any) {
